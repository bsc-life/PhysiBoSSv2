...
void update_custom_variables( Cell* pCell )
{
	...
	pCell->custom_data.variables.at(index_ainhib_node).value = pCell->boolean_network.get_node_value("anti_A");
	...
	pCell->custom_data.variables.at(index_binhib_node).value = pCell->boolean_network.get_node_value("anti_B");
}
...
void set_input_nodes(Cell* pCell) {
	...
	if (ainhib_index != -1)
	{
		...
		if (ainhib_cell_concentration >= ainhib_threshold)
			pCell->boolean_network.set_node_value("anti_A", 1);
		else
		{
			pCell->boolean_network.set_node_value("anti_A", 0);
		}
	}
		...
		if (binhib_index != -1)
	{
		...
		if (binhib_cell_concentration >= binhib_threshold)
			pCell->boolean_network.set_node_value("anti_B", 1);
		else
		{
			pCell->boolean_network.set_node_value("anti_B", 0);
		}
	}
}
...
void from_nodes_to_cell(Cell* pCell, Phenotype& phenotype, double dt)
{
	...
	// For template_BM model
	std::string prosurvival_basename = "C";
	double prosurvival_value = 0.0;

	bn_index = pCell->boolean_network.get_node_index( prosurvival_basename );
	if ( (*nodes)[bn_index] > 0)
	{
